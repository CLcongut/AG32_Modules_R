...
/***************************************************************************************
 * Encoding with UTF-8
 * 本程序移植自江协科技 OLED 驱动程序 V1.2 ，使用模拟I2C
 * 适配AG32VF系列，使用宏定义关闭原模拟I2C部分
 * 目前版本为：V1.0
 *
 * 本程序由江协科技创建并免费开源共享
...
#define OLED_W_SDA(x) (x == 1) ? GPIO_SetHigh(I2C_SW_GPIO_PORT, I2C_SW_SDA) : GPIO_SetLow(I2C_SW_GPIO_PORT, I2C_SW_SDA)
#define OLED_W_SCL(x) (x == 1) ? GPIO_SetHigh(I2C_SW_GPIO_PORT, I2C_SW_SCL) : GPIO_SetLow(I2C_SW_GPIO_PORT, I2C_SW_SCL)

void OLED_GPIO_Init(void)
{
	UTIL_IdleMs(1000);
	SYS_EnableAPBClock(I2C_SW_GPIO_MASK);
	GPIO_SetOutput(I2C_SW_GPIO_PORT, I2C_SW_SDA | I2C_SW_SCL);
	GPIO_SetHigh(I2C_SW_GPIO_PORT, I2C_SW_SDA | I2C_SW_SCL);
}
